...
CRedisClient::~CRedisClient()
{
    ...
    m_bExit = true;
    {
		std::lock_guard<std::mutex> safeLock(m_rwLock);
...
void CRedisClient::operator()()
{
    ...
    {
        {
			std::unique_lock<std::mutex> safeLock(m_rwLock);
            if (m_bValid)
...
{
    {
		std::unique_lock<std::mutex> safeLock(m_rwLock, std::try_to_lock_t());
        if (safeLock.owns_lock())
            m_condAny.notify_all();
    }
...
{
	std::unique_lock<std::mutex> safeLock(m_rwLock, std::try_to_lock_t());
    if (!safeLock.owns_lock() || !m_bValid)
        return RC_RQST_ERR;
